...
## Update 2026-07-20 (ter) — Layout, cover, compliance CrazyGames finale

- **Layout title screen**: `.overlay` scrollabile (`overflow-y:auto` + trick margin-auto su primo/ultimo figlio: centrato quando entra, scroll quando no); media query `max-height:820px` comprime h1/gap; rimosso doppio scroll di `#skin-row`. Prima su schermi bassi lo shop era tagliato sotto il fold.
- **Badge CrazyGames**: `assets/branding/play_cg.png` (dal brand kit, ridimensionato 300px) in fondo al title screen, visibile SOLO quando `onCrazyGames` (il gioco non è esclusivo: su altri host non appare).
- **Cover** (requisiti https://docs.crazygames.com/requirements/game-covers/) in `assets/covers/`: `cover_16x9_1920x1080.png`, `cover_2x3_800x1200.png`, `cover_1x1_800x800.png`. gpt-image-1 quality high (chiave in `giochi/.env`), titolo stilizzato incluso, no bordi/loghi/testo extra, estetica coerente (capra bianca, mondo low-poly, monete). Landscape: raw 1536x1024 → crop centrale 16:9 → 1920x1080; portrait 1024x1536 (=2:3) → 800x1200; square 1024 → 800.
- **Checklist requisiti verificata** (docs.crazygames.com): peso totale 5MB (limiti 50 init/250 tot) ✓, 89 file (≤1500) ✓, SDK v3 + gameplayStart/Stop + loading events ✓, ads solo via SDK + fallback AdBlock ✓, PEGI12 ✓, Escape mai intercettato ✓, `e.code` posizionale → AZERTY/ZQSD funziona nativamente ✓, bottoni chiari senza delay ✓, onboarding: hint visivi sul titolo + toast esplicativi in gioco ✓.

## Update 2026-07-20 (quater) — Data Module SDK, upload build, form submission

- **Persistenza via CrazyGames Data Module** (`store` wrapper in main.js, sostituisce ogni `localStorage.*` diretto): stessa firma `getItem/setItem` di localStorage per design (docs.crazygames.com/sdk/data). Motivo: warning ufficiale del form submission — "Automatic Progress Save" (localStorage) NON funziona nei giochi in iframe, e tutti i giochi CrazyGames girano in iframe. `store.get/set` usa `CG().data` quando `sdkReady` è true, altrimenti fallback diretto a `localStorage` (dev locale, altri host).
- **Riordino boot critico**: prima i `let totalCoins = +(localStorage.getItem(...))` ecc. leggevano SINCRONI a module-eval time, prima che l'SDK potesse inizializzarsi — con Data Module servirebbe leggere dati vuoti/sbagliati per utenti loggati CrazyGames. Fix: variabili dichiarate con default, popolate da nuova `loadPersistedState()` chiamata SOLO dopo `Promise.all([sdkReadyPromise, loadLibrary()])` nel boot finale, prima di `resetWorld()`/`renderShop()`. `sdkInit()` ora imposta `sdkReady=true` a fine esecuzione ed è awaitato (`sdkReadyPromise`) invece di fire-and-forget.
- Testato in browser con vero script SDK CrazyGames caricato (`window.CrazyGames.SDK.data.setItem/getItem` reali, non stub): scrittura+reload → valore letto correttamente dal Data Module, non da localStorage.
- **Form submission CrazyGames**: "Upload files" NON accetta zip — vuole i file/cartelle sciolti trascinati direttamente nella dropzone (errore osservato: "Archive files are not supported"). Cartella pronta in `upload-to-crazygames/` (index.html + css/ js/ vendor/ assets/{audio,branding,icons,models}, NO assets/covers NO assets_src NO brand kit inutilizzato) — drag di tutto il contenuto della cartella (non la cartella stessa se il picker non la accetta, provare prima con la cartella intera). "Does your game save progress?" → **"Yes, using the Data Module from the CrazyGames SDK"** (ora corretto, prima si sarebbe scelto LocalStorage per errore).
- **Mobile orientation** → Landscape (camera ortografica larga, 9 corsie giocabili, verticale le taglierebbe). **Multiplayer** → No (client-side puro, zero networking).

## Update 2026-07-20 (quinquies) — Fix card MONDI, mute piattaforma SDK

- **Fix card WORLDS a doppia riga** (`css/style.css`): nomi bioma lunghi ("BOSCO D'AUTUNNO") andavano a 2 righe e spingevano `.skin-cost`/`.unlock-row` fuori dalla card (`overflow:hidden` li tagliava, "400"/"AD" illeggibili). Fix: `.skin-name` e `.skin-perk` ora hanno `min-height` fissa (2 righe riservate sempre, che il testo sia 1 o 2 righe) così ogni card nella stessa vista ha lo stesso layout verticale; card `.skin-btn` 144px→160px per margine di sicurezza.
- **Mute audio piattaforma** (docs.crazygames.com/sdk/game/#game-settings): il mute del sito/tab CrazyGames non fermava l'audio del gioco (SDK non wired). Aggiunto `CG().game.addSettingsChangeListener` → `platformMuted`, letto anche una volta a boot (`sdk.game.settings.muteAudio`). Gated in `tone()`, `noise()`, `playSfx()` e su `bgMusic` (`startMusic`/`setMusicOn`/`applyPlatformMute`) — **priorità sul toggle audio in-game**, come richiesto dai docs: se la piattaforma muta, il toggle interno non può riaccendere il suono. Testato end-to-end in browser con SDK reale: flag toggle → SFX bloccati (`playSfx` ritorna null) → musica in pausa → tutto ripristinato al rilascio. Checkbox form "supports CrazyGames muting audio through SDK" → ora spuntabile onestamente. NB: il listener va registrato DENTRO `sdkInit().then()`, non prima, altrimenti "CrazySDK is not initialized yet".

## Update 2026-07-20 (sexies) — Bottone MENU game over, SDK init gating

- **Bottone "🏠 MENU & SHOP" nella schermata game over** (`backToMenu()`): riporta al title screen (shop 3 tab, scelta modalità, scelta mondo di partenza) con `resetWorld()`+`renderShop()` così skin/gear/mondo appena comprati sono già equipaggiati. Prima da game over si poteva SOLO riprovare/revive/condividere — nessun accesso a shop/modalità/scenari tra una run e l'altra. i18n key `menu_btn`.
- **SDK init gating**: ogni `sdk*()` guard (gameplayStart/Stop/happy/loading + midroll/rewarded + `store`) ora fa `if (!sdkReady) return` e `sdkReady` diventa true SOLO se `init()` risolve davvero (prima veniva settato true anche nel catch). Off-platform (localhost puro) init() rigetta → nessuna chiamata `game.*`/`data.*` → niente spam "CrazySDK is not initialized yet". Listener settings mute e `sdkLoadingStart()` spostati dentro `sdkReadyPromise.then()` (init prima di ogni altra chiamata SDK, requisito CrazyGames). In ambiente QA/sandbox con SDK reale: init riesce, Data Module attivo, chiamate gameplay pulite (4 "not initialized" residui = rumore interno SDK durante handshake init, non generati dal nostro codice, assenti on-platform).

## Update 2026-07-20 (septies) — Scroll touch overlay su mobile

- **Problema mobile**: title e game over più alti dello schermo (specie landscape su telefono) venivano tagliati e NON scrollabili col dito; inoltre qualsiasi tap sul title avviava subito la run (in `touchstart`), rendendo impossibile scorrere/usare il negozio. Il gioco resta giocabile sia landscape che portrait (l'SDK CrazyGames mostra da sé il suggerimento "ruota il telefono" in portrait), quindi si lascia così — serviva solo rendere i menu usabili al touch.
- **Fix touch** (`main.js`): avvio run spostato da `touchstart` a `touchend` con distinzione tap-vs-swipe (soglia 18px, come già in gameplay): tap su spazio vuoto avvia, swipe scorre il menu senza avviare, tap su card/modalità/bottoni gestito dal loro click (helper `onMenuControl` su `e.target` reale del touch). Listener `touchstart`/`touchend` resi `passive:true` (scroll nativo libero); `touchmove` fa `preventDefault` SOLO durante `state==='playing'` (blocca lo scroll pagina in gioco, invariato) e lo lascia libero su title/game over.
- **Fix CSS** (`.overlay`): `touch-action: pan-y` + `-webkit-overflow-scrolling: touch` per riabilitare il drag-scroll verticale nonostante `body { touch-action: none }` (necessario al gameplay). Overlay già `overflow-y:auto`.
- Verificato in browser (mobile landscape 740×360, con SDK reale): overlay scrollabile fino in fondo (Deer/Crazy Goat/"tocca per iniziare" raggiungibili), swipe non avvia, tap avvia, tap su card/modalità non avvia, bottone MENU del game over raggiungibile via scroll. Regressioni gameplay controllate: swipe sinistra/destra/su muovono la capra come prima, tap = avanti. Nessuna regressione.

## Update 2026-07-20 (octies) — Bottone PLAY esplicito

- **Problema**: su web il click del mouse NON avviava (solo `keydown` avviava il title, il mouse non genera keydown), e il vecchio "tap/premi ovunque per iniziare" confliggeva con la navigazione del menu (ogni tap avviava). Scelta: **bottone PLAY dedicato** (`#play-btn` "▶ GIOCA"), grande e pulsante, sotto i due bottoni modalità. Uniforme web+mobile, menu/negozio completamente navigabile con click/tap/scroll.
- **Avvio ora SOLO da**: click/tap sul bottone PLAY, oppure tasto Enter/Space da tastiera. Rimosso l'avvio "any key" (i tasti freccia sul title non avviano più → non interferiscono col menu) e l'avvio "tap ovunque" dal touchend (il touchend ora gestisce SOLO gli hop in `state==='playing'`; su title/game over il touch resta libero di scorrere). i18n `play_btn`; `press_start` riconvertito in "or press ENTER".
- Verificato web+mobile: click su card/tab/sfondo NON avvia (naviga), GIOCA avvia, Enter avvia, freccia su title non avvia, swipe mobile scorre senza avviare, ciclo game over→MENU→GIOCA ok, gameplay (tastiera+swipe) invariato. Nessuna regressione.

## Update 2026-07-20 (nonies) — Fix gap cielo negli angoli (ground floor)

- **Problema**: avanzando, nell'angolo vicino (basso-destra, peggio su schermi larghi) compariva un triangolo azzurro (lo `scene.background` cielo) dove il terreno generato non arrivava — le righe dietro vengono cullate (`BEHIND`) e le lane coprono solo ±21 in x, così il frust: iso scopre il vuoto nell'angolo near.
- **Fix** (`main.js`): grande piano `floor` (PlaneGeometry 200×200, MeshLambert) a `y=-0.35` (sotto la water lane), `renderOrder=-1`, che segue la camera (`floor.position.z=-camRow`) e viene tinto ogni frame col colore erba del bioma corrente via `groundColorAt(score)` — helper PURO (nessun `genRand`, non perturba il seed della daily). Costruito una volta (`buildFloor()` in `resetWorld`), aggiornato in `updateFloor()` nel loop. Qualunque gap ai bordi ora mostra terreno, mai cielo. La fog fa sfumare il floor lontano all'orizzonte (naturale).
- Verificato via readPixels su schermo largo (1600×740): angolo basso-destra SENZA floor = `[89,143,163]` (blu = cielo), CON floor = `[84,112,51]` (verde = terreno). Gap eliminato. Zero errori console, nessun z-fight (floor sotto il box lane più profondo).

## Update 2026-07-20 (decies) — Economia chiarita, bonus giornaliero, record, floor ottimizzato

- **Valuta unica chiarita**: c'è UNA sola valuta (🪙 `totalCoins`), raccolta in-run + wallet, compra tutto. Niente doppia valuta (scelta deliberata: split = grind+confusione su un hopper). Riequilibrio prezzi per renderli raggiungibili (~24 monete/100 righe + ora bonus distanza): accessori 60–900 (erano 150–2000), skin abilità 1200–3500 (erano 5000–12000), mondi 400. Nuovo **bonus distanza**: +10🪙 ogni 25 righe (celebrate `"N! +10🪙"`), così le run lunghe pagano davvero.
- **Bonus giornaliero** (`#daily-bonus-screen`, chiave `capra_dailybonus_YYYYMMDD` via store): overlay opaco sopra il title mostrato una volta al giorno, 3 cespugli 🌳 → tap → reveal ricompensa pesata: monete comuni (30–60 al 55%, 120 al 20%), rara scudo/vita alla prossima run (`pendingStartShield`/`pendingStartLife` applicati in `startGame`), rarissima accessorio o mondo non posseduto. `rollDailyReward`/`applyDailyReward`. Bottone PLAY chiude → title. Testato: reward applicato, monete +50, claim persistito, non riappare al reload.
- **Sezione RECORD** (4° tab shop 🏆): Distanza record (`best`), Record del giorno (`dailyBest`), Run più lunga (`bestTime`, salvato alla morte), Partite giocate (`runsPlayed`, ++ a `startGame`), Monete guadagnate (`coinsEarned`, cumulativo lifetime). Stat persistite (`saveStats`/`capra_besttime|runs|earned`).
- **Floor ottimizzato**: colore aggiornato SOLO al cambio bioma (`floorBiome` guard in `updateFloor`), non ogni frame; resettato a `-1` in `resetWorld`. La camera segue sempre (invariato).
- Tutte le nuove stringhe UI in i18n (8 lingue): tab_records, rec_*, daily_bonus_*, db_*.
